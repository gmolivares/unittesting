...
class UserTests(unittest.TestCase):
    ...
    def test_user_with_multiple_accounts(self):
        ...
        for _ in range(3):
            banck_account=BankAccount(
                balance=self.faker.random_int(min=100,max=2000,step=50),log_file=self.faker.file_name(extension="txt")
            )
            self.user.add_account(account=banck_account)
